...
\end{equation*}
Nhưng, để đơn giản hóa, tác giả sẽ bỏ qua việc xây dựng các định nghĩa bậc cao hơn. Điều đó cũng cho phép chúng ta lạm dụng kí hiệu. Chẳng hạn, chúng ta có thể viết $x = y = z$ với nghĩa $x = y \land y = z$, chứ không phải $(x = y) = z$ hay $x = (y = z)$.
...
\begin{enumerate}
   ...
   \item $\forall x, \forall y, ((y = A \implies x \in y) \iff x \in A)$.
\end{enumerate}
...
\stepcounter{subexercise}
\arabic{subexercise}. Chúng ta sẽ chứng minh thông qua hai chiều. Đầu tiên, giả sử $y = A \implies x \in y$ với mọi $x$ và $y$. Cụ thể hóa $y$ để có
\begin{equation*}
   A = A \implies x \in A.
\end{equation*} 

Có $A = A$ luôn đúng do tính phản xạ của phép bằng nhau, cho nên phải có $x \in A$. Chiều thuận này được chứng minh.
