...
\begin{multicols}{2}
   \begin{enumerate}
      ...
      \item $A \setminus B = A \iff A \cap B = \emptyset$;
      \item $A \setminus B = \emptyset \iff A \subseteq B$;
...
\stepcounter{subexercise}
\arabic{subexercise}. Lấy một phần tử $x$ bất kỳ thuộc $A$. Xét hai trường hợp xảy ra đối với tập hợp $B$:
\begin{itemize}
...
\stepcounter{subexercise}
\arabic{subexercise}. Trước hết, giả sử $A \subseteq B$. Lấy một phần tử $x$ bất kỳ thuộc vào tập hợp $A \setminus C$. Theo định nghĩa của phép hiệu tập hợp,
$ x \in A \land x \notin C. $
...
\arabic{subexercise}. Chứng minh chiều thuận từ $A \subseteq B$ sang $\begin{cases}
   A \cap C \subseteq B \cap C \\
   A \setminus C \subseteq B \setminus C
\end{cases}$ đã được tác giả trình bày ở những bài tập trước. Chứng minh chiều ngược lại, giả sử có $\begin{cases}
   A \cap C \subseteq B \cap C \\
   A \setminus C \subseteq B \setminus C
\end{cases}$