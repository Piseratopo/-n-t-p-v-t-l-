...
$$
   \defMath{\forall x, \exists y, \forall z, \left( z \in y \iff z \in x \land \text{\textbf{\cauF}}(z) \right)}.
$$
...
Trước khi đi vào những vấn đề khác, chúng ta sẽ trình bày về một cách viết tắt các mệnh đề trong giải tích vị từ. Bạn đọc có thể để ý, trong định nghĩa, chúng ta có một mệnh đề dạng
$$
...
\end{equation*}
Tuy rằng định nghĩa này nhìn không giống so với định nghĩa đối với lượng từ phổ quát, cách hiểu này là hợp lí với cách dùng ngôn ngữ thông thường. Khi nói ``tồn tại'' thuộc đâu đó, chúng ta luôn mặc định rằng có một đối tượng nằm trong vị trí đó với tính chất đã có, ít khi hiểu theo hướng có một đối tượng \emph{nếu} nằm trong đó thì có tính chất đó.
...
\begin{equation*}
   \exists ! a \in x, V(a) \iff \exists ! a, (a \in x \land V(a)).
\end{equation*}
Chúng ta có biến đổi
\begin{align*}
   \exists ! a \in x, V(a) &\iff \exists ! a, (a \in x \land V(a)); \\
   \exists ! a \in x, V(a) &\iff \exists a, \left(a \in x \land V(a) \land \forall b, (b \in x \land V(b) \implies b = a)\right) \\
   & \qquad \coloringCrimson{\text{(định nghĩa tồn tại duy nhất)}}; \\
   \exists ! a \in x, V(a) &\iff \exists a, \left(a \in x \land V(a) \land \forall b, (b \in x \implies (V(b) \implies b = a)) \right) \\
   & \qquad \coloringCrimson{(P \land Q \implies R) \iff (P \implies (Q \implies R))}; \\
   \exists ! a \in x, V(a) &\iff \exists a \in x, \left(V(a) \land \forall b, (b \in x \implies (V(b) \implies b = a)) \right); \\
   \exists ! a \in x, V(a) &\iff \exists a \in x, \left(V(a) \land \forall b \in x, (V(b) \implies b = a) \right).
\end{align*}
...
\begin{equation*}
   y = \defMath{\{ z \in x \mid \text{\textbf{\cauF}}(z) \}} = \defMath{\{ z \in x : \text{\textbf{\cauF}}(z) \}}.
\end{equation*}
...
Thông thường, để dễ hình dung các phép toán trên tập hợp, tác giả sẽ giới thiệu về cách biểu diễn tập hợp bằng sơ đồ Ven\footnote{John Venn (1834 -- 1923).}. Với hai tập hợp $x$ và $y$, chúng ta có thể biểu diễn chúng bằng hai hình chồng lên nhau, trong đó kết quả của các phép tính trên chúng được tô màu, với một sự minh hoạ cụ thể ở hình \ref{fig:li_thuyet_tap_hop:tien_de:tien_de_tach:ven_phep_tinh}.
...
Để kết thúc, tác giả sẽ định nghĩa một khái niệm thường xuyên được thấy trong tổ hợp. Với tập hợp $x$, gọi $P \in \mathfrak{P}\left(\mathfrak{P}(x)\right)$. $P$ là \defText{phân hoạch} của $x$ khi và chỉ khi 
\begin{equation*}
   \begin{cases}
      \defMath{\forall p \in P, p \neq \varnothing} \\
      \defMath{\forall p \in P, \forall q \in P, (p \neq q \implies p \cap q = \varnothing)} \\
      \defMath{\forall y \in x, \exists p \in P, y \in p}
   \end{cases}.
\end{equation*}
...
\begin{multicols}{2}
   \begin{enumerate}
      ...
      \item $\begin{cases}
         A \subseteq B \\
         A \neq \varnothing
      \end{cases} \implies \bigcap A \supseteq \bigcap B$;
...
\stepcounter{subexercise}
\arabic{subexercise}. Nhắc lại, $A \cap B$ là tập hợp được lấy ra từ tiên đề tách với tập ban đầu là $A$ và vị từ là ``$() \in B$''. Do đó, theo hệ quả \eqref{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:he_qua_tap_con} đã có ở trang \pageref{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:he_qua_tap_con}, $A \cap B \subseteq A$. Điều phải chứng minh.
...
\stepcounter{subexercise}
\arabic{subexercise}. Giả sử $A \subseteq B$ và $A \neq \varnothing$. Xét $z$ là một phần tử thuộc $\bigcap B$. Khi đó, $z \in b$ với mọi $b \in B$. Do $A \subseteq B$, mọi phần tử $x \in A$ cũng là một phần tử của $B$. Dẫn đến, viết lại các mệnh đề để có
\begin{equation*}
    \begin{cases}
...
\end{equation*}
Nhưng, nhận thấy rằng, $\forall x \in A, z \in x \implies z \in \bigcup A$ là đúng với mọi $z$. Thật vậy, do $A \neq \varnothing$, tồn tại $x^*$ để $x^* \in A$, dẫn đến $z \in x^*$. Từ tiên đề hợp, chúng ta có $z \in \bigcup A$ từ $x^* \in A \land z \in x^*$ hay $\exists x, x \in A \land z \in x$.
...
Bảng giá trị chân lí \ref{tab:li_thuyet_tap_hop:tien_de:tien_de_tach:7} cho phép chúng ta suy ra từ \eqref{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:7}:
\begin{equation*}
...
\stepcounter{subexercise}
\arabic{subexercise}. Nếu \coloringCrimson{$A \neq \varnothing$} thì $B$ cũng khác rỗng do $A = B$. Dưới điều kiện đó, chúng ta đã chứng minh:
\begin{equation*}
...
Nếu \coloringDodgerblue{$A = \varnothing$} thì $B$ cũng bằng $\varnothing$. Từ đó, có $\begin{cases}
   A \subseteq C \\
   B \subseteq C
\end{cases}$, và dễ dàng tính được
\begin{equation*}
   \begin{cases}
      \bigcap A = \bigcap B = \varnothing \\
...
\setcounter{subexercise}{1}
\arabic{subexercise}. Có $A \setminus B = \{x \in A \mid x \notin B\}$. Theo hệ quả của tiên đề tách \eqref{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:he_qua_tap_con}, $A \setminus B \subseteq A$. Điều phải chứng minh.
...
\begin{itemize}
    \item Nếu $x \in B$, vì $x \in A$ và $x \in B$, chúng ta có $x \in A \cap B$.\\
...
Từ \eqref{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:so_sanh_hieu.1} và \eqref{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:so_sanh_hieu.2}, chúng ta có điều phải chứng minh.
...
\begin{itemize}
   ...
   Vậy, luôn tồn tại phần tử $x$ không thỏa mãn $x \in A \vartriangle C \iff x \in B \vartriangle C$. Tuy nhiên, điều này mâu thuẫn với $A \vartriangle C = B \vartriangle C$.
...
\exercise Cho $\mathscr{E}$ là một tập hợp, $P$ là một phân hoạch của $\mathscr{E}$, $A$ là một tập con của $P$ và $B = \complement_P(A)$. Đặt $\mathscr{F} = \{x \in \mathscr{E} \mid \exists a \in A, x \in a\}$ và $\mathscr{G} = \{x \in \mathscr{E} \mid \exists b \in B, x \in b\}$.
\begin{enumerate}
   \item Chứng minh rằng $A$ là một phân hoạch của $\mathscr{F}$ và $B$ là một phân hoạch của $\mathscr{G}$.
   \item Chứng minh rằng $\mathscr{F} \cup \mathscr{G} = \mathscr{E}$ và $\mathscr{F} \cap \mathscr{G} = \varnothing$\footnote{Bạn đọc có thể tự hỏi rằng tại sao tác giả không viết ngắn gọn hơn là ``$\{\mathscr{F}; \mathscr{G}\}$ là một phân hoạch của $\mathscr{E}$''. Để một tập có thể là phân hoạch thì các phần tử của nó phải khác rỗng. Trong điều kiện được cho của đề bài, nếu $A$ (hoặc $B$) rỗng thì $\mathscr{F}$ (hoặc tập tương ứng là $\mathscr{G}$) cũng rỗng.}. 
\end{enumerate}

\solution

\resetSubexercise Có $A$ là tập con của $P$, cho nên với mọi phần tử $a$ thuộc $A$ cũng đều nằm trong $P$. Do đó, $a$ không rỗng. Vậy
\begin{equation}
   \forall a \in A, a \neq \varnothing.
   \label{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:dk_khong_rong}
\end{equation}

Gọi $b$ và $c$ cùng thuộc $A$ sao cho $b \neq c$. Thấy được rằng $b$ và $c$ là hai phần tử khác nhau đều nằm trong $P$, theo định nghĩa của phân hoạch $P$, $b \cap c = \varnothing$. Vì thế,
\begin{equation}
   \forall (b; c) \in A^2, (b \neq c \implies b \cap c = \varnothing).
   \label{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:dk_khong_giao}
\end{equation}

Từ cách đặt của $\mathscr{F}$, chúng ta dễ thấy rằng 
\begin{equation}
   \forall d \in \mathscr{F}, \exists e \in A, d \in e.
   \label{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:dk_thuoc}
\end{equation}

Ngoài ra, lấy $q \in A$ bất kì và $r \in a$ bất kì. Do $A \subseteq P$ nên $q \in P$. Thêm vào đó, $P$ là phân hoạch của $\mathscr{E}$, suy ra $q \subseteq \mathscr{E}$, dẫn đến $r \in \mathscr{E}$. Vì $r \in q$ và $q \in A$, theo định nghĩa của $\mathscr{F}$, có $r \in \mathscr{F}$. Do đó, $\forall r \in q, r \in \mathscr{F}$, hay $q \subseteq \mathscr{F}$, tức là $q \in \mathfrak{P}(\mathscr{F})$. Vì $q \in A$ được lấy tùy ý, chúng ta có
\begin{equation*}
   A \subseteq \mathfrak{P}(\mathscr{F}).
\end{equation*} Kết hợp kết quả vừa có với \eqref{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:dk_khong_rong}, \eqref{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:dk_khong_giao} và \eqref{eq:li_thuyet_tap_hop:tien_de:tien_de_tach:dk_thuoc}, chúng ta đã chứng minh được $A$ là một phân hoạch của $\mathscr{F}$. $B$ là một phân hoạch của $\mathscr{G}$ có thể được chứng minh theo một cách tương tự.

\stepSubexercise Hiển nhiên rằng $\mathscr{F} \cup \mathscr{G} \subseteq \mathscr{E}$ do $\begin{cases}
   \mathscr{F} \subseteq \mathscr{E}\\
   \mathscr{G} \subseteq \mathscr{E}
\end{cases}$. Xét một phần tử $f$ bất kì trong $\mathscr{E}$. Giả sử $f \notin \mathscr{F} \cup \mathscr{G}$, suy ra, 
\begin{equation*}
   \begin{cases}
      \overline{f \in \mathscr{E} \land \exists g \in A, f \in g} \\
      \overline{f \in \mathscr{E} \land \exists g \in B, f \in g} 
   \end{cases}.
\end{equation*}
Mà, $f \in \mathscr{E}$ nên $\begin{cases}
   \forall g \in A, f \notin g \\
   \forall g \in B, f \notin g
\end{cases}$, có nghĩa là $\forall g, \left((g \in A \implies f \notin g) \land (g \in B \implies f \notin g)\right)$ hay $$
   \forall g, \left[\begin{aligned}
      g &\in A \\
      g &\in B
   \end{aligned}\right. \implies f \notin g.
$$ Vì $A \subseteq P$ và $B = \complement_P(A)$ nên $A \cup B = P$, cho chúng ta $\forall g, \left[\begin{aligned}
   g &\in A \\
   g &\in B
\end{aligned}\right. \iff g \in P$. Qua đó, chúng ta có \begin{equation*}
   \forall g, g \in P \implies f \notin g.
\end{equation*}
Tuy nhiên, điều này mâu thuẫn với giả thuyết $P$ là một phân hoạch của $\mathscr{E}$. Chứng minh bằng phản chứng, chúng ta cần phải có $\forall f, (f \in \mathscr{E} \implies f \in \mathscr{F} \cup \mathscr{G})$. Vì vậy, $\mathscr{E} = \mathscr{F} \cup \mathscr{G}$.

Tiếp theo, chúng ta chứng minh rằng $\mathscr{F} \cap \mathscr{G} = \varnothing$. Giả sử ngược lại, tồn tại $h$ sao cho $h$ thuộc đồng thời $\mathscr{F}$ và $\mathscr{G}$, theo cách đặt của hai tập hợp này, tồn tại $i \in A$ và $j \in B$ sao cho $h \in i \land h \in j$. Khi này $i \cap j \neq \varnothing$ do $h \in i \cap j$. Tuy nhiên, nhận ra rằng $i$ và $j$ theo cách gọi đều là phần tử thuộc phân hoạch $P$, nên khi kết hợp với $i \cap j \neq \varnothing$ thì có được $i = j$, khiến cho $A \cap B$ khác rỗng do có $i$ (hay $j$) trong nó. Mặc dù vậy, trong cách gọi của đề bài, $B = \complement_P(A)$ sẽ cho chúng ta $A \cap B = \varnothing$, tạo mâu thuẫn. 

Từ chứng minh bằng phản chứng, chúng ta có $\mathscr{F} \cap \mathscr{G} = \varnothing$. Kết hợp với $\mathscr{F} \cup \mathscr{G} = \mathscr{E}$ chứng minh từ trước, chúng ta có điều phải chứng minh.