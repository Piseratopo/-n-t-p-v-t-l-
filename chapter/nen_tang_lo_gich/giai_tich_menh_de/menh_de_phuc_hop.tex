\section{Mệnh đề phức hợp và các phép nối mệnh đề}
...
\begin{itemize}
   ...
   \item Phép \defText{phủ định kéo theo} -- $\defMath{\nRightarrow}$ hoặc $\defMath{\nrightarrow}$, $\defMath{\centernot{\implies}}$, $\defMath{\centernot{\rightarrow}}$, $\defMath{\not\supset}$;
   \item Phép \defText{phủ định hệ quả} -- $\defMath{\nLeftarrow}$ hoặc $\defMath{\nleftarrow}$, $\defMath{\centernot{\impliedby}}$, $\defMath{\centernot{\leftarrow}}$, $\defMath{\not\subset}$;
   \item Phép \defText{phủ định đẳng giá} -- $\defMath{\nLeftrightarrow}$ hoặc $\defMath{\nleftrightarrow}$, $\defMath{\centernot{\iff}}$, $\defMath{\centernot{\leftrightarrow}}$; 
   \item Phép \defText{tuyển chọn} hoặc phép \defText{tuyển lại trừ} -- $\defMath{\oplus}$ hoặc $\defMath{\veebar}$, $\defMath{\centernot{\leftrightarrow}}$\footnote{Kí hiệu giống với phủ định đẳng giá không phải là do trùng hợp.}, $\defMath{\not\equiv}$;
...
\end{itemize}
}, để xác định giá trị chân lí của mệnh đề có nhiều phép nối, các phép nối mệnh đề cũng có sắp xếp mức ưu tiên cao đến ưu tiên thấp:
\begin{enumerate}[start=0]
   \item Thực hiện các phép bên trong cặp dấu ngoặc --- $()$ --- trước, ngoài ngoặc sau. Sau khi đã xác định được vị trí trong ngoài, chúng ta thực hiện các phép theo thứ tự:
...
\end{itemize} Cho nên, khi viết các mệnh đề phức hợp dưới dạng biểu thức, không nên coi thứ tự được đưa ra ở đây như một chân lí và bỏ qua những dấu ngoặc cần thiết, nhưng cũng không nên dùng thừa dấu gây khó đọc.}. Ví dụ:
\begin{itemize}
...
\begin{table}[H]
   ...
   \label{tab:nen_tang_lo_gich:giai_tich_menh_de:menh_de_phuc_hop:ket_hop_hoi}
...
\begin{table}[H]
   ...
   \label{tab:nen_tang_lo_gich:giai_tich_menh_de:menh_de_phuc_hop:ket_hop_tuyen}
   \begin{tabular}{|c|c|c|ccccccccccc!{\vrule width 1.5pt}c|}
      \hline
...
Hai bảng \ref{tab:nen_tang_lo_gich:giai_tich_menh_de:menh_de_phuc_hop:ket_hop_hoi} và \ref{tab:nen_tang_lo_gich:giai_tich_menh_de:menh_de_phuc_hop:ket_hop_tuyen} cho phép chúng ta viết $P \land Q \land R$ và $P \lor Q \lor R$ mà không phải lo lắng về thứ tự các phép tính. Điều này có thể mở rộng lên nhiều biến hơn, dẫn đến chúng ta có thể viết 
\begin{equation*}
   \left\{ \begin{array}{c}
      P_1 \\
      P_2 \\
      \vdots \\
      P_n
   \end{array} \right. \text{ hoặc } P_1 \land P_2 \land \cdots \land P_n
\end{equation*}
và 
\begin{equation*}
   \left[\begin{array}{c}
      ...
      \vdots\\
      P_n
   \end{array}\right. \text{ hoặc } P_1 \lor P_2 \lor \cdots \lor P_n
\end{equation*}
...
Tương tự như trước, chúng ta có thể viết $P \iff Q \iff R$, nhưng đừng.
...
Quy tắc này có thể được mở rộng lên nhiều mệnh đề hơn:\begin{equation*}
   \left\{ \begin{array}{c}
      P_1 \implies R \\
      P_2 \implies R \\
      \vdots \\
      P_n \implies R
   \end{array} \right. \iff \left(\left[\begin{array}{c}
      P_1 \\
      P_2 \\
      \vdots \\
      P_n
   \end{array} \implies R\right.\right).
\end{equation*}
